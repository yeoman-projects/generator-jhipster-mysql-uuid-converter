        ...
        return {
            updateFiles() {
                ...
                const javaDir = `${jhipsterConstants.SERVER_MAIN_SRC_DIR +
                    this.packageFolder}/`;
                const javaTestDir = `${jhipsterConstants.SERVER_TEST_SRC_DIR +
                    this.packageFolder}/`;
                ...
                if (!pattern.test(content)) {
                    ...
                    if (
                        fs.existsSync(
                            `${javaDir}service/mapper/${entityName}Mapper.java`
                        )
                    ) {
                        this.longToUUID(
                            `${javaDir}service/mapper/${entityName}Mapper.java`
                    ...
                    if (
                        fs.existsSync(
                            `${javaDir}repository/search/${entityName}SearchRepository.java`
                        )
                    ) {
                        this.longToUUID(
                            `${javaDir}repository/search/${entityName}SearchRepository.java`
                    ...
                    // Resource
                    this.longToUUID(
                        `${javaDir}web/rest/${entityName}Resource.java`
                    );

                    // JavaScript
                    const entityNameSpinalCased = _s.dasherize(
                        _s.decapitalize(entityName)
                    );
                    ...
                    const file = glob.sync(
                        `src/main/resources/config/liquibase/changelog/*entity_${entityName}.xml`
                    )[0];
